...
doc <:doc<
   ...
   tactic applies its argument and fails if $@i[tac]$ succeeds and
   produces subgoals.}
   @end[description]
...
doc <:doc< ************************************************************************
   ...
   @rulebox{selT; 2@space (@tt[dT]@space 0);
     <<sequent(nil){ <H>; x: 'T_2; <J> >- <:doc<T_1@space @i{Type}>>}>> @cr
     <<sequent(nil){ <H>; x: 'T_2; <J> >- 'T_2}>>;
     <<sequent(nil){ <H>; x: 'T_2; <J> >- <:doc<T_1 @vee T_2>>}>>}
   $$}
   @end[description]
...
doc <:doc< ************************************************************************
   ...
   (@i[tac] $i_1$ @tt[thenT] $@cdots$ @tt[thenT] @i[tac] $i_n$ @tt[thenT] @i[tac] $0$).  The
