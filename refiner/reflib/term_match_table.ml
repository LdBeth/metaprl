 ...
 *                   specified number of hypotheses and the conclusion.
 *
...
let rec compile_sequent prog stack = function
 ...
 | Hypothesis (_, t) :: hyps ->
      begin match compile_sequent prog stack hyps with
       ...
       | ProgSeq(Right, _) :: _, _        -> raise bad_sequent_patt
       | ProgSeq (kind, i) :: prog, stack -> ProgSeq(kind, (i+1)) :: prog, t :: stack
       | _                                -> raise compseq_bug
      end
 | Context _ :: hyps ->
      begin match compile_sequent prog stack hyps with
...
let rec compile info prog = function
 ...
 | mk :: t :: ts when mk == sequent_marker ->
      ...
      if SeqGoal.length eseq.sequent_goals <> 1 then
         raise(Invalid_argument "Term_match_table.compile: multiconclusion sequents not supported");
      let hyps = SeqHyp.to_list eseq.sequent_hyps in
      let prog, stack = compile_sequent prog ((SeqGoal.get eseq.sequent_goals 0)::ts) hyps in
...
and seq_add_to_choice prog knd i lst =
   match lst, knd with
      (((Exact|Left), _, _) as hd :: tl), Right
    | ((Exact, _, _) as hd :: tl), Left ->
 ...
 | DtreeSeq(lst) ->
      begin match stack with
         t :: ts ->
            ...
            let stack' = (SeqGoal.get eseq.sequent_goals 0) :: ts in
               execute_sequent search fallbacks stack eseq.sequent_hyps l stack' lst
       | [] ->
            raise (Invalid_argument "Term_match_table.execute: bug")
      end
...
and execute_sequent search fallbacks old_stack hyps l stack = function
 ...
 | (knd, i, tbl) :: lst ->
      ...
      in
         execute_hyps search fallbacks tbl hyps stack i last
