...
module MakeRefinerDebug (Refiner1 : RefinerSig) (Refiner2 : RefinerSig) = struct
   ...
   module TermOp = struct
      ...
      let map_up f (t1, t2) =
         let f1 t1 = fst (f (term_of_term1 t1)) in
         let f2 t2 = snd (f (term_of_term2 t2)) in
            merge_term "TermOp.map_up" (TermOp1.map_up f1 t1) (TermOp2.map_up f2 t2)

      let map_down f (t1, t2) =
         let f1 t1 = fst (f (term_of_term1 t1)) in
         let f2 t2 = snd (f (term_of_term2 t2)) in
            merge_term "TermOp.map_down" (TermOp1.map_down f1 t1) (TermOp2.map_down f2 t2)
