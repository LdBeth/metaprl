 ...
 * List of ints.
 *)
let rec format_ints buf = function
   [] -> ()
 | [i] -> format_int buf i
 | h::t ->
      format_string buf "$";
...
(*
 * Print ints with names.
 *)
let format_intstrings buf ints names =
...
let rec format_names buf = function
   [h] ->
      begin
         match h with
            SaveName n ->
               format_quoted_string buf n
          | ArgName i ->
               format_int buf i
          | StackName i ->
               format_int buf i
 ...
 *)
let rec dform_format_terms buf = function
   [] -> ()
 | [h] -> format_simple_term buf h
 | h::t ->
      format_simple_term buf h;
      format_string buf ";";
      format_space buf;
      dform_format_terms buf t

(*
 * Print a term.
 *)
let rec format_term buf = function
   RWComposite { rw_op = { rw_name = opname; rw_params = params } ; rw_bterms = bterms } ->
      format_pushm buf 3;
      format_szone buf;
      format_quoted_string buf (string_of_opname opname);
      if params <> [] then
         begin
            format_string buf "{";
 ...
 | RWSOMatch (i, (ivars, svars, terms)) ->
      format_pushm buf 3;
      format_string buf "$<match ";
      format_int buf i;
      format_string buf ">[";
      format_intstrings buf ivars svars;
      format_string buf ". ";
      dform_format_terms buf terms;
      format_string buf "]";
      format_popm buf

 | RWSOSubst (i, l) ->
      format_pushm buf 3;
      format_string buf "$<subst ";
      format_int buf i;
      format_string buf ">[";
      format_terms buf l;
      format_string buf "]";
 ...
 | RWSOContextSubst (i, t, l) ->
      format_pushm buf 3;
      format_string buf "$<cosubst ";
      format_int buf i;
      format_string buf ">[";
      format_term buf t;
      if l <> [] then
 ...
 | RWCheckVar i ->
      format_string buf "$<check ";
      format_int buf i;
      format_string buf ">"

 | RWStackVar i ->
      format_string buf "$<newvar ";
      format_int buf i;
      format_string buf ">"
...
and format_bterm buf = function
   { rw_bvars = 0; rw_bnames = _; rw_bterm = bterm } ->
      format_term buf bterm

 | { rw_bvars = vars; rw_bnames = names; rw_bterm = bterm } ->
      if vars <> List.length names then
         begin
            format_string buf "@";
            format_int buf vars;
            format_string buf "="
         end;
      format_names buf names;
      format_string buf ".";
      format_space buf;
      format_term buf bterm
   ...
   and compile_so_redex_param stack param =
      ...
      in
         match dest_param param with
          ...
          | Var(v) -> stack, RWVar(v)
          | _ -> raise (BadMatch (ParamMatch param))
...
let build_contractum names stack =
   ...
   let rec build_contractum_term bvars = function
    ...
    | t -> raise (RewriteError "bad contractum")
       ...
       | RWSum (p1, p2) ->
            begin
                match (build_contractum_param p1, build_contractum_param p2) with
                 ...
                 | (Number i, p) -> raise (BadMatch (ParamMatch (make_param p)))
                 | (p, _) -> raise (BadMatch (ParamMatch (make_param p)))
            end
       | RWDiff (p1, p2) ->
            begin
                match (build_contractum_param p1, build_contractum_param p2) with
   ...
   if debug_rewrite then
      begin
         let buf = new_buffer () in
            ...
            format_terms buf redex;
            format_newline buf;
            format_string buf "seq: ";
            dform_format_terms buf terms;
            format_newline buf;
   ...
   let gstack = Array.create gstacksize StackVoid in
	 ...
	 match contractum with
	  ...
	  | RWCFunction f ->
               match terms with
                  [t] -> [f t], names'
                | _ -> raise (BadMatch (TermMatch xnil_term))
