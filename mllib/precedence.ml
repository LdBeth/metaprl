 ...
 *
 * $Log$
 * Revision 1.1  1998/04/08 14:57:25  jyh
 * ImpDag is in mllib.
 *
...
(*
 * Actual precedence has a name
 * and an index into the precedence DAG.
 *)
type t =
   { prec_name : string;
     prec_index : int
...
let min_prec =
   { prec_name = "!min_precedence";
     prec_index = 0
   }

let max_prec =
   { prec_name = "!max_precedence";
     prec_index = 1
   }
