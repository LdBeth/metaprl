...
let search_server dir =
   ...
   let rec search i =
      if i = max_servers then
         raise (Failure "Mmap_pipe.search_server: too many running servers");
...
let create_server dir =
   ...
   in
   let file = Mmap.create bufname [Unix.O_RDWR; Unix.O_CREAT] 438 in
   let buf = Mmap.to_string file in
   let _ =
      if !debug_pipe then
   ...
   let sock = Unix.socket Unix.PF_UNIX Unix.SOCK_STREAM 0 in
   let at_exit () =
      eprintf "Mmap_pipe: cleaning up server sockets%t" eflush;
...
let search_client dir =
   let rec search i =
      if i = max_servers then
         raise (Failure "Mmap_pipe: no servers are online");
      let filename = sprintf "%s/%d" dir i in
      let sockname = filename ^ sock_suffix in
      let sock = Unix.socket Unix.PF_UNIX Unix.SOCK_STREAM 0 in
         try
...
let create_client dir =
   ...
   let bufname = filename ^ buffer_suffix in
   let file = Mmap.create bufname [Unix.O_RDWR] 438 in
   let buf = Mmap.to_string file in
   let _ =
      if !debug_pipe then
         eprintf "Buffer size: 0x%08x %d%t" (Obj.magic buf) (String.length buf) eflush
...
let open_client mmap =
   match mmap with
    ...
    | _ ->
         raise (Invalid_argument "Mmap_pipe.open_client")
...
let close_client mmap =
   eprintf "Mmap_pipe: closing client%t" eflush;
   match mmap with
...
let block mmap =
   if !debug_pipe then
      eprintf "Mmap_pipe.block: 0x%08x%t" (Obj.magic mmap.mmap_data) eflush;
...
let write mmap code name raw_write =
   if !debug_pipe then
      eprintf "Mmap_pipe.write: begin 0x%08x%t" (Obj.magic mmap.mmap_data) eflush;
   match mmap.mmap_socket with
      Some sock ->
         ...
         let full = get_int buf (woffset + full_offset) in
            if full = 0 then
               ...
               begin
                  if !debug_pipe then
                     eprintf "Mmap_pipe.write: failed%t" eflush;
                  false
               end
    | None ->
         raise (Invalid_argument "Mmap_pipe.write")
...
let read mmap raw_read =
   if !debug_pipe then
      eprintf "Mmap_pipe.read: begin 0x%08x%t" (Obj.magic mmap.mmap_data) eflush;
   match mmap.mmap_socket with
      Some sock ->
         ...
         let full = get_int buf (roffset + full_offset) in
            if full = 0 then
               begin
                  if !debug_pipe then
                     eprintf "Mmap_pipe.read: failed%t" eflush;
                  None
               end
            else
               ...
               let data = raw_read buf (roffset + offset) data_length in
                  ...
                  Some (code, name, data)
    | None ->
         raise (Invalid_argument "Mmap_pipe.read")
