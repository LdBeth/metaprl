...
(*
 * These namses must be unique
 * XXX: BUG? (nogin): we shouldn't really trust this
 * location stuff to give us a unique id.
 *)
let action_of_id id =
   ...
   in
      sprintf "(%d,%d)%s" line (cnum - bol) filename
...
struct
   ...
   let pp_print_action = pp_print_string

   module ActionSet = StringSet;;

   let choose = max
end;;

module ActionTable = StringTable;;
...
let add_token gram id s contractum_opt =
   ...
   in
   let action = action_of_id id in
   let arity, lexer = Lexer.add_clause lexer action s in
   let rw_opt =
      match contractum_opt with
       ...
       | None ->
            None
   in
   let actions = ActionTable.add actions action rw_opt in
...
let add_production gram id args opt_prec contractum =
   ...
   let parse = Parser.add_production parse action name ops opt_prec in
   let redex = mk_simple_term parser_arg_opname args in
   let rw = term_rewrite Relaxed [||] [redex] [contractum] in
   let actions = ActionTable.add actions action rw in
      { gram with gram_name           = None;
                  gram_parser         = parse;
                  gram_parser_actions = actions
      }
...
let add_iform gram id redex contractum =
   ...
   let action = action_of_id id in
      { gram with gram_name = None;
                  gram_iforms = ActionTable.add gram.gram_iforms action iform;
                  gram_iform_table = None
      }
