...
struct
    ...
    * Inherit proof_type.
    *)
   type proof_type = Cache.proof_type
   ...
   let extract_str_item proc (item, loc) =
      match item with
       ...
       | Axiom ({ axiom_name = name; axiom_proof = Cache.Primitive t } as ax) ->
            if debug_filter_prog then
               eprintf "Filter_prog.extract_str_item: prim axiom: %s%t" name eflush;
            prim_axiom proc loc ax t
       | Axiom ({ axiom_name = name; axiom_proof = Cache.Derived tac } as ax) ->
            if debug_filter_prog then
               eprintf "Filter_prog.extract_str_item: thm axiom: %s%t" name eflush;
            thm_axiom proc loc ax tac
       | Axiom ({ axiom_name = name; axiom_proof = Cache.Interactive tac } as ax) ->
            if debug_filter_prog then
