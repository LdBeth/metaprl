...
let add_quot name check =
   ...
   let quot_patt s =
      ...
      let t = unchecked_term_of_parsed_term dummy_loc t in
         Filter_exn.print_exn Dform.null_base (Some "Can not build a pattern out of a term:\n") Filter_patt.build_term_patt t
   in
      Quotation.add name (Quotation.ExAst (quot_exp, quot_patt))
...
let rec mk_list_expr loc = function
   [] -> <:expr< [] >>
 | hd :: tl -> <:expr< [ $hd$ :: $mk_list_expr loc tl$ ] >>
...
let expr_of_pcon loc = function
   ConPStr s, shape ->
      let shape =
         match shape with
            ShapeString -> "String"
          | _ ->
               Ploc.raise loc (Invalid_argument "\"con\" quotation: string constant parameter must be of string kind")
      in
 ...
 | ConPMeta s, shape ->
      let shape =
         match shape with
          ...
          | _ -> Ploc.raise loc (Invalid_argument "\"con\" quotation: unsupported meta-parameter")
      in
 ...
 | ConPNum _, _
 | ConPInt _, _ ->
      Ploc.raise loc (Invalid_argument "\"con\" quotation: numeric parameter of non-numeric kind?")
...
EXTEND
      ...
      [[ interf_opening; st = LIST0 interf_item; EOI ->
          let f () =
             ...
             let id = SigFilter.hash proc in
                SigFilter.add_command proc (Id id, loc);
                SigFilter.save proc AnySuffix;
                SigFilter.extract () proc
          in
             handle_exn f "interf" loc, Some loc
       ]];
          ...
          let f () =
             ...
             let () =
                ...
                if get_checked_bindings loc <> [] then
                   Ploc.raise loc (Invalid_argument "Filter_parse.interf_item: sig item has bindings");
                match s with
                   <:sig_item< declare $list: []$ end >> ->
                      ()
                 | _ ->
   ...
   implem:
      [[ implem_opening; st = LIST0 implem_item; EOI ->
          ...
          in
             handle_exn f "implem" loc, Some loc
       ]];
          ...
          let f () =
             ...
             let () =
                match s with
                   <:str_item< declare $list: []$ end >> ->
                      if get_checked_bindings loc <> [] then
                         Ploc.raise loc (Invalid_argument "Filter_parse.implem_item: empty str item has bindings")
                 | _ ->
   ...
   sig_item:
      [[ "extends"; path = mod_ident ->
        ...
        | "define"; sc = shapeclasses; name = LIDENT; res = optresources; ":"; quote = quote_term; "<-->"; def = term ->
          let f () =
             ...
             let quote, def = parse_define_term loc name sc quote def in
                SigFilter.define_term proc loc sc name quote def res false
           in
        ...
        | "rule"; name = LIDENT; args = optarglist; ":"; mt = mterm ->
           let f () =
              let proc = SigFilter.get_proc loc in
              let _, t, args, _ = TermGrammar.parse_rule loc name mt args in
      ...
      [[ "extends"; path = mod_ident ->
        ...
        | "define"; opaque = opaque_flag; sc = shapeclasses; name = LIDENT; res = optresources; ":"; quote = quote_term; "<-->"; def = term ->
          let f () =
             ...
             let quote, def = parse_define_term loc name sc quote def in
                StrFilter.define_term proc loc sc name quote def res opaque
           in
              handle_exn f ("define " ^ name) loc;
              empty_str_item loc
        | "declare"; "rewrite"; redex = term; "<-->"; contractum = term ->
        ...
        | mlrule_keyword; name = LIDENT; res = optresources; args = optarglist; ":"; t = parsed_bound_term; "="; code = expr ->
           let f () =
              let proc = StrFilter.get_proc loc in
              let args = List.map (parse_term loc) args in
              let code = bind_item loc code in
      ...
      [[ flag = opt_pvt_flag; e = expr LEVEL "expr1" ->
            let rec split_application tl expr =
               match expr with
                ...
                | <:expr< $lid:name$ >> ->
                     { res_loc = loc; res_name = name; res_flag = flag; res_args = tl }
                | _ ->
       ...
       | mt = bmterm ->
            ...
            with
               Ploc.Exc (_, Failure _) ->
                  Ploc.raise loc (Failure "No computational witness (\"extract\") specified for a prim rule and the default_extract{} opname is not declared")
      ]];
