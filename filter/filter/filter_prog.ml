...
let rewrite_ctyp loc =
   <:ctyp< $longid:rewrite_type_longid loc$ . conv >>
...
let refiner_ctyp loc =
   <:ctyp< $longid:refiner_longid loc$ . refiner >>
...
let expr_of_loc loc =
   ...
   let first_line = Ploc.line_nb loc in
   let first_bol = Ploc.bol_pos loc in
...
let empty_args_spec loc =
   <:expr< Refiner.Refiner.Rewrite.empty_args_spec >>

let args_val_expr loc i_ids a_ids =
   if i_ids = [] && a_ids = [] then
      empty_rw_args loc
   else
...
let args_spec_expr loc ivars avars =
   if ivars = [] && avars = [] then
      empty_args_spec loc
   else
...
let expr_of_label loc = function
 ...
 | [h] ->
      <:expr< Some $str: h$ >>
 | _ ->
      Ploc.raise loc (Invalid_argument "Filter_prog.expr_of_label: multi-component labels not supported")
 ...
 *)
let parent_path_longid loc = function
   h::t ->
      List.fold_left (fun li s -> <:extended_longident:< $longid:li$ . $uid:String.capitalize_ascii s$ >>)
    <:extended_longident< $uid:String.capitalize_ascii h$ >> t
 | [] ->
...
let declare_cond_rewrite loc { crw_name = name; crw_params = params; _ } =
   [<:sig_item< value $name$ : $params_ctyp loc (cond_rewrite_ctyp loc) params$ >>]

let declare_ml_rewrite loc { mlterm_name = name; mlterm_params = params; _ } =
   [<:sig_item< value $name$ : $params_ctyp loc (cond_rewrite_ctyp loc) params$ >>]

(*
 * Rules.
 *)
let declare_rule loc { rule_name = name; rule_params = params; _ } =
   [<:sig_item< value $name$ : $params_ctyp loc (tactic_ctyp loc) params$ >>]

let declare_ml_axiom loc { mlterm_name = name; mlterm_params = params; _ } =
   [<:sig_item< value $name$ : $params_ctyp loc (tactic_ctyp loc) params$ >>]
...
let declare_toploop_item loc item =
   begin match item with
       ...
       | _ ->
            Ploc.raise loc (RefineError ("declare_toploop_item", StringError "illegal topval"))
   end;
   [item]
 ...
 *)
let declare_magic_block loc { magic_code = items; _ } =
   items
...
let flag_expr loc = function
   Private -> <:expr< Mp_resource.Private >>
 | Public -> <:expr< Mp_resource.Public >>

let impr_resource proc loc flag name expr =
   let ctyp, fqn = find_res proc loc name in
      <:expr< Mp_resource.improve $flag_expr loc flag$ $str:fqn$ (Obj.repr ( $expr$ : $ctyp$ )) >>

let impr_resource_list proc loc flag name expr =
   let ctyp, fqn = find_res proc loc name in
      <:expr< Mp_resource.improve_list $flag_expr loc flag$ $str:fqn$ (Obj.magic ( $expr$ : list $ctyp$ )) >>
...
let impr_toploop proc loc name pvt_flag (expr,texpr) =
   let expr = <:expr< ($str:proc.imp_name$, $str: name$, $expr$, $texpr$) >> in
      <:str_item< ($impr_resource proc loc pvt_flag "toploop" expr$) >>
...
let toploop_rewrite proc loc name pvt_flag params =
   let base body = <:expr< Shell_sig.ConvExpr $body$ >>, <:expr< Shell_sig.ConvType >> in
      impr_toploop proc loc name pvt_flag (loop_params loc 0 <:expr< $lid: name$ >> base params)

let toploop_rule proc loc name params =
   let base body = <:expr< Shell_sig.TacticExpr $body$ >>, <:expr< Shell_sig.TacticType >> in
      impr_toploop proc loc name Public (loop_params loc 0 <:expr< $lid: name$ >> base params)
 ...
 *)
let define_rewrite_resources proc loc name force_private redex contractum assums addrs params resources name_id_expr =
   if resources.item_item = [] then
      <:expr< () >>
   else
   let define_resource {res_loc = loc; res_name = name'; res_flag = flag; res_args = args} =
   let flag = if force_private then Private else flag in
      let processor = apply_annotation_processor loc <:expr< $lid:"process_" ^ name' ^ "_resource_rw_annotation"$ >> args in
         impr_resource_list proc loc flag name' <:expr<
            $processor$ $str:name$ $redex$ $contractum$ $assums$ $addrs$ $params$ $expr_of_loc loc$ $name_id_expr$
         >>
   in
 ...
 *)
let define_rewrite want_checkpoint force_private code proc loc rw expr =
   ...
   let create_rw = <:expr<
      ...
      in do {
         $prim_expr$;
         $define_rewrite_resources proc loc name force_private redex contractum nil (empty_args_spec loc) nil rw.rw_resources rw_id_expr$;
         $rewrite_of_pre_rewrite_expr loc$ $rw_id_expr$ $empty_rw_args loc$ $nil$
      }
    >> in
...
let define_cond_rewrite want_checkpoint code proc loc crw expr =
   ...
   let resources =
      if crw.crw_assums == [] then
         ...
         { crw.crw_resources with item_item = List.map (add_crw_selector loc) crw.crw_resources.item_item }
   in
   let create_expr = <:expr<
      let $lid:args_id$ = $args_spec_expr loc ivars avars$ in
      let $lid:params_id$ = $list_expr loc (expr_of_term proc loc) tparams$ in
      let $lid:subgoals_id$ = $list_expr loc (expr_of_term proc loc) crw.crw_assums$ in
      ...
      in do {
         $prim_expr$;
         $define_rewrite_resources proc loc name false redex contractum subgoals_expr args_expr params_expr resources rw_id_expr$;
         $rw_id_expr$
      }
    >> in
    let rw_fun_expr =
        fun_expr loc all_ids <:expr<
           $rewrite_of_pre_rewrite_expr loc$ $rw_id_expr$ $args_val_expr loc ivar_ids avar_ids$ $list_expr loc lid_expr tparam_ids$ >>
    in
       checkpoint_resources want_checkpoint loc name [
...
let prim_rewrite proc loc rw =
   define_rewrite false false (prim_rewrite_expr loc) proc loc rw None
...
let define_term proc loc class_term def =
   ...
   in
      define_rewrite false def.term_def_opaque (def_rewrite_expr loc) proc loc rw None
...
let derived_rewrite proc loc rw expr =
   define_rewrite true false (derived_rewrite_expr loc) proc loc rw (Some expr)
...
let interactive_rewrite proc loc rw =
   define_rewrite true false (delayed_rewrite_expr loc) proc loc rw (Some (extract_expr loc proc.imp_name rw.rw_name))
...
let define_ml_rewrite proc loc mlrw rewrite_expr =
   ...
   let addrs_spec, addrs_val, create_ml_rewrite_expr =
      if simple_flag then
         ...
         let ivars, avars, tparams = split_params params in
            list_expr loc (expr_of_term proc loc) tparams,
            <:expr< unknown >>,
   ...
   let rewrite_body = <:expr<
      let $lid:stack_id$ = $apply_redex_expr loc$ $rewrite_id_expr$ $addrs_val$ $lid:goal_id$ $params_expr$ in $rewrite_body$
   >> in
   let args_ids =
      if simple_flag then
         [goal_id]
   ...
   let body = <:expr<
      ...
      do {
         $define_rewrite_resources proc loc name false redex_id_expr nil_term (<:expr< [] >>) addrs_spec params_expr mlrw.mlterm_resources rewrite_id_expr$;
         $rewrite_id_expr$
      }
   >> in
      [
 ...
 * along with the params, so that we can figure out its type.
 *)
let define_rule_resources proc loc name args_id params_id assums_id resources name_rule_expr =
   if resources.item_item = [] then <:expr< () >> else
   let define_resource {res_loc = loc; res_name = name'; res_flag = flag; res_args = args} =
      let processor = apply_annotation_processor loc <:expr< $lid:"process_" ^ name' ^ "_resource_annotation"$ >> args in
         impr_resource_list proc loc flag name' <:expr<
            $processor$ $str:name$ $lid:args_id$ $lid:params_id$ $lid:assums_id$ $expr_of_loc loc$ $name_rule_expr$
         >>
   in
      bindings_let proc loc resources <:expr< do { $list:List.map define_resource resources.item_item$ } >>
...
let define_rule prim_rule deffun proc loc
   ...
   let name_value =
      fun_expr loc all_ids <:expr<
         $tactic_of_rule_expr loc$ $name_rule_expr$ $args_val_expr loc ivar_ids avar_ids$ $list_expr loc lid_expr tparam_ids$
      >>
   in
   ...
   let rule_expr = <:expr<
      ...
      let $lid:rule_id$ =
         $refiner_expr loc$.create_rule $lid:local_refiner_id$ $str:name$ $lid:args_id$ $lid:params_id$ $lid:assums_id$
      in
      ...
      in
      let _ = do {
         $refiner_expr loc$ . $lid:deffun$ $lid:local_refiner_id$ $str:name$ $lid:args_id$ $lid:params_id$ $lid:assums_id$ $extract_args$ $extract$;
         $define_rule_resources proc loc name args_id params_id assums_id resources name_rule_expr$
      }
      in
         $name_rule_expr$
      >>
...
let define_ml_rule want_checkpoint proc loc
   ...
   in
   let rule_let  = <:expr<
      let $lid:rule_id$ = $fun_expr loc [addrs_id; goal_id; params_id] rule_body$ in
   ...
   let body = <:expr<
      let $lid:redex_id$ =
         Refiner.Refiner.Rewrite.compile_redices Rewrite_sig.Strict $args_spec_expr loc ivars avars$
   ...
   let rule_fun_expr =
      fun_expr loc all_ids <:expr<
         $tactic_of_rule_expr loc$ $lid:name_rule_id$ $args_val_expr loc ivar_ids avar_ids$ $list_expr loc lid_expr tparam_ids$
      >>
   in
      checkpoint_resources want_checkpoint loc name (**)
...
let define_ml_dform proc loc
   ...
   >> in
      [<:str_item<
         ...
         let $lid:redex_id$ =
            ($rewriter_expr loc$ .compile_redices) Rewrite_sig.Relaxed $empty_args_spec loc$ [ $lid:term_id$ ]
         in
...
let improve_resource proc loc { improve_name = name; improve_flag = flag; improve_expr = expr } =
   let expr' = expr_of_bnd_expr proc loc expr in
   let improve_expr =
      if is_list_expr expr.item_item then
         impr_resource_list proc loc flag name expr'
      else
         impr_resource proc loc flag name expr'
   in
...
let define_parent proc loc
   ...
   let parent_path = parent_path_longid loc path in
   ...
   let make_ctyp (name, _) =
      let path = find_resource name proc.imp_all_resources in
         (name, (<:ctyp< $longid:parent_path_longid loc path$ . $lid:input_type name$ >>, res_fqn path name))
   in
...
let add_toploop_item proc loc name ctyp =
   let expr = toploop_item_expr loc name ctyp in
      impr_toploop proc loc name Public expr
 ...
 * An regular item.
 *)
let wrap_toploop_item proc loc ((patt, expr, attr) as item) =
   match patt with
      <:patt< $lid:name$ >> when List.mem_assoc name proc.imp_toploop ->
         let ctyp = get_top_ctyp proc name in
         let expr = wrap_toploop_item loc name ctyp expr in
            (patt, expr, attr), [add_toploop_item proc loc name ctyp]
     | _ ->
         item, []

let wrap_toploop_items proc loc lpea =
   let lpea, resources = List.split (List.map (wrap_toploop_item proc loc) lpea) in
...
let rec wrap_summary_items proc = function
 ...
 | (item, loc)::items ->
      ...
      begin match item with
         <:str_item< value $flag:rec_flag$ $list:lpe$ >> ->
            let lpe, toploop = wrap_toploop_items proc loc lpe in
            let toploop = List.map (fun x -> x, loc) toploop in
               (<:str_item< value $opt:rec_flag$ $list:lpe$ >>, loc) :: (toploop @ items)
       | <:str_item< external $name$ : $ctyp$ = $list:_$ >> when List.mem_assoc name proc.imp_toploop ->
            (item, loc) :: (add_toploop_item proc loc name (get_top_ctyp proc name), loc) :: items
       | _ ->
         (item, loc)::items
      end
