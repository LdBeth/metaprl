          ...
          | "minus"
            [ "-"; n = INT ->
                let n = Lm_num.neg_num (Lm_num.num_of_string n) in
                    wrap_term loc (mk_term (mk_op (mk_opname loc ["number"] [ShapeNumber] []) [make_param (Number n)]) [])
            | "-"; x = SELF ->
                wrap_term loc (mk_dep0_term (mk_dep0_opname loc "minus") (make_term x))
            ]
          | "prod" RIGHTA
          ...
          | "raw" RIGHTA
            ...
            | op = opname_list; ":"; t = SELF ->
              match op with
               ...
               | _ ->
                    Ploc.raise loc (ParseError "illegal binding variable")
            ]
                ...
                let aux = fun r -> function
                      (Some lab,t) ->
                           mk_term (mk_op (mk_opname loc ["record"] [ShapeToken] [1;0])
                               [make_param (Token (mk_token_opname loc lab))])  [mk_bterm [Lm_symbol.add "self"]
                               (get_aterm loc t); mk_simple_bterm  r]
                   |  (None,t) ->
           ...
           | "{"; f =  word_or_string; "|"; t = aterm; "}" ->
             ...
             let rfun_op = mk_dep0_dep2_opname loc "rfun" in
                if is_dep0_dep1_term (mk_dep0_dep1_opname loc "fun") t then
                   ...
                   Ploc.raise loc (ParseError "body of <rfun> is not a function")
            | "!"; v = var ->
               encode_free_var v
            | x = QUOTATION ->
