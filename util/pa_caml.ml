(* camlp5r *)
(* This file is forked from CamlP5 pa_o.ml *)
...
value mklistexp _loc last =
  loop True where rec loop top =
    ...
    | [e1 :: el] ->
        let _loc =
          if top then _loc else Ploc.encl (MLast.loc_of_expr e1) _loc
        in
...
value mklistpat _loc last =
  loop True where rec loop top =
    ...
    | [p1 :: pl] ->
        let _loc =
          if top then _loc else Ploc.encl (MLast.loc_of_patt p1) _loc
        in
...
value concat_comm _loc e =
  let _loc =
    Ploc.with_comment _loc
      (Ploc.comment _loc ^ Ploc.comment (MLast.loc_of_expr e))
  in
  let floc =
    let first = ref True in
    fun loc1 ->
      if first.val then do {first.val := False; _loc}
      else loc1
  in
  Reloc.expr floc 0 e
;
...
value expr_of_patt p =
  ...
  | _ -> Ploc.raise _loc (Stream.Error "identifier expected") ]
;
 ...
 ;
  ...
  expr:
    ...
    | "simple" LEFTA
      ...
      | "("; e = SELF; ")" -> concat_comm _loc <:expr< $e$ >>
      | "begin"; e = SELF; "end" -> concat_comm _loc <:expr< $e$ >>
      | "begin"; "end" -> <:expr< () >>
      | x = QUOTATION ->
          let con = quotation_content x in
          Pcaml.handle_expr_quotation _loc con ] ]
  ;
  let_binding:
  ...
  patt:
    ...
    | "simple"
      ...
      | x = QUOTATION ->
          let con = quotation_content x in
          Pcaml.handle_patt_quotation _loc con ] ]
  ;
  patt_semi_list:
    [ [ p = patt; ";"; pl = SELF -> [p :: pl]
      | p = patt; ";" -> [p]
  ...
  constructor_declaration:
    [ [ ci = cons_ident; "of"; cal = V (LIST1 (ctyp LEVEL "apply") SEP "*") ->
      ...
      | ci = cons_ident; ":"; cal = V (LIST1 (ctyp LEVEL "apply") SEP "*") ->
          ...
          in
          (_loc, ci, <:vala< [] >>, Some t)
      | ci = cons_ident -> (_loc, ci, <:vala< [] >>, None) ] ]
  ;
  cons_ident:
    [ [ i = V UIDENT "uid" "" -> i
  ...
  class_declaration:
    [ [ vf = V (FLAG "virtual"); ctp = class_type_parameters; i = V LIDENT;
        cfb = class_fun_binding ->
          {MLast.ciLoc = _loc; MLast.ciVir = vf; MLast.ciPrm = ctp;
  ...
  class_type_parameters:
    [ [ -> (_loc, <:vala< [] >>)
      | "["; tpl = V (LIST1 type_parameter SEP ","); "]" -> (_loc, tpl) ] ]
  ;
  class_fun_def:
      ...
      | "val"; ov = V (FLAG "!") "!"; mf = V (FLAG "mutable");
        "virtual"; lab = V LIDENT "lid" ""; ":"; t = ctyp ->
          if Pcaml.unvala ov then
            Ploc.raise _loc (Stream.Error "virtual value cannot override")
          else
  ...
  EXTEND
    ...
    joinautomaton:
      [ [ jcl = V (LIST1 joinclause SEP "or") ->
            {MLast.jcLoc = _loc; MLast.jcVal = jcl} ] ]
    ;
    joinclause:
    ...
    ;
    joinpattern:
      [ [ ji = joinident; "("; op = V (OPT patt); ")" -> (_loc, ji, op) ] ]
    ;
    joinident:
      [ [ i = V LIDENT -> (_loc, i) ] ]
    ;
...
EXTEND
  ...
  interf:
    [ [ si = sig_item_semi; (sil, stopped) = SELF -> ([si :: sil], stopped)
      | "#"; n = LIDENT; dp = OPT expr; ";;" ->
          ([(<:sig_item< # $lid:n$ $opt:dp$ >>, _loc)], None)
      | EOI -> ([], Some _loc) ] ]
  ;
  sig_item_semi:
  ...
  implem:
    [ [ si = str_item_semi; (sil, stopped) = SELF -> ([si :: sil], stopped)
      | "#"; n = LIDENT; dp = OPT expr; ";;" ->
          ([(<:str_item< # $lid:n$ $opt:dp$ >>, _loc)], None)
      | EOI -> ([], Some _loc) ] ]
  ;
  str_item_semi:
