...
let incr_level_exp = function
   ({ le_const = c; le_vars = vars } : levelExp) ->
      let add1 = function
...
let max_level_exp = fun
   ({ le_const = c1; le_vars = l1 } : levelExp)
   ({ le_const = c2; le_vars = l2 } : levelExp) ->
...
let is_so_var_term = function
   ...
   when opname == var_opname ->
       for_all (function { bvars = [] } -> true | _ -> false) bterms
 | _ -> false;;
...
let dest_so_var = function
   ({ term_op = { op_name = opname; op_params = [Var(v)] };
      ...
      v, List.map (function { bvars = []; bterm = t } -> t | _ -> raise (TermMatch term)) bterms
 | term -> raise (TermMatch term);;
...
let dest_context = function
   ({ term_op = { op_name = opname; op_params = [Var v] };
      ...
      v, term', List.map (function { bvars = []; bterm = t } -> t | _ -> raise (TermMatch term)) bterms
  | term -> raise (TermMatch term);;
...
let term_subterm term = function
   (Path addr) as a ->
      begin
         let rec aux t = function
            [] -> t
          | i::tl -> aux (nth t.term_terms i).bterm tl
         in
...
let rec standardize_apart_term i bvars gvars = function
   { term_op = { op_name = opname; op_params = [Var v] }; term_terms = bterms } when opname == var_opname ->
      (* Variable *)
      let i', v', gvars' =
         if mem_assoc v bvars then
            i, assoc v bvars, gvars
         else if mem_assoc v gvars then
            i, assoc v gvars, gvars
         else
            i + 1, i + 1, (v, i + 1)::gvars
      in
...
and standardize_apart_param i gvars = function
    ...
    *)
   MNumber(v) ->
      if mem_assoc v gvars then
         i, gvars, MNumber(new_bvar (assoc v gvars))
      else
         i + 1, (v, i + 1)::gvars, MNumber(new_bvar (i + 1))
 | MString(v) ->
      if mem_assoc v gvars then
         i, gvars, MString(new_bvar (assoc v gvars))
      else
         i + 1, (v, i + 1)::gvars, MString(new_bvar (i + 1))
 | MToken(v) ->
      if mem_assoc v gvars then
         i, gvars, MToken(new_bvar (assoc v gvars))
      else
         i + 1, (v, i + 1)::gvars, MToken(new_bvar (i + 1))
 | MLevel(v) ->
      if mem_assoc v gvars then
         i, gvars, MLevel(new_bvar (assoc v gvars))
      else
         i + 1, (v, i + 1)::gvars, MLevel(new_bvar (i + 1))
 | MVar(v) ->
      if mem_assoc v gvars then
         i, gvars, MVar(new_bvar (assoc v gvars))
      else
...
let rec free_vars_term gvars bvars = function
   { term_op = { op_name = opname; op_params = [Var(v)] }; term_terms = bterms } when opname == var_opname ->
      (* This is a variable *)
      let gvars' = if mem v bvars or mem v gvars then
                      gvars
                   else
                      v::gvars
      in
...
and binding_vars_bterms bvars = function
   { bvars = vars; bterm = t }::l ->
      binding_vars_bterms (binding_vars_term (union vars bvars) t) l
 | [] -> bvars;;
...
let compare_operators = fun
   ...
   { op_name = name2; op_params = params2 } ->
      if name1 = name2 then
         compare_lists compare_params params1 params2
...
let compare_pattern_operators = fun
   ...
   { op_name = name2; op_params = params2 } ->
      if name1 = name2 then
         compare_lists compare_pattern_params params1 params2
      else if name1 < name2 then
         -1
      else
         1;;
...
let alpha_equal_vars (t, v) (t', v') =
   try equal_term (zip v v') t t' with
      Invalid_argument _ -> false;;
...
let equal_comp vars' =
   let rec equal_comp_term vars = function
      { term_op = { op_name = opname; op_params = [Var v] };
        term_terms = []
         ...
         begin
            try equal_term vars' t' (assoc v vars) with
               Not_found ->
                  begin
                     match t' with
                        { term_op = { op_name = opname; op_params = [Var v'] };
   ...
   and equal_comp_bterms vars bterms1 bterms2 =
      let equal_comp_bterm = fun
         { bvars = bvars1; bterm = term1 }
         { bvars = bvars2; bterm = term2 } ->
            equal_comp_term (zip_list vars bvars1 (List.map mk_var_term bvars2)) (term1, term2)
      in
...
let subst term terms vars =
   let rec subst_term terms fv vars = function
      { term_op = { op_name = opname; op_params = [Var(v)] }; term_terms = [] } as t when opname == var_opname->
         (* Var case *)
         begin
             try let i = find_index vars v in nth terms i with
                Not_found -> t
         end
    | { term_op = op; term_terms = bterms } ->
         (* Other term *)
   ...
   and subst_bterms terms fv vars bterms =
      ...
      let rec subst_bterm = function
       ...
       | { bvars = bvars; bterm = term } ->
            ...
            let flags = List.map (function v -> mem v bvars) vars in
            let vars' = remove_elements vars flags in
            let fv' = remove_elements fv flags in
            let terms' = remove_elements terms flags in
   ...
   and subst_bvars renames' renames bvars =
      let subst_bvar v =
         try (let i = find_index renames v in nth renames' i) with
            Not_found -> v
      in
...
let rec unify_terms subst bvars = function
   ...
   when opname == var_opname ->
      ...
      begin
         try
            if (assoc v bvars) = (dest_var t2) then
               subst
            else
         ...
         with
            Not_found ->
               begin
                  try unify_terms subst bvars (assoc v subst, t2) with
                     Not_found ->
                        (v, t2)::subst
               end
          | TermMatch _ ->
   ...
   when opname == var_opname ->
      ...
      begin
         ...
         with
            Not_found ->
               begin
                  try unify_terms subst bvars (t1, assoc v subst) with
                     Not_found ->
                        (v, t1)::subst
               end
          | TermMatch _ ->
