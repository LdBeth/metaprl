...
mk/config: mk/make_config.sh
	...
	@ROOT="$(ROOT)" TERMS="$(TERMS)" REFINER="$(REFINER)" FORMAT="$(FORMAT)" MAKE_OPTS="$(MAKE_OPTS)" SEQ_SET="$(SEQ_SET)" CCC="$(CCC)" ENSROOT="$(ENSROOT)" OCAMLSRC="$(OCAMLSRC)" THEORIES="$(THEORIES)" TESTS_ENABLED="$(TESTS_ENABLED)" READLINE_ENABLED="$(READLINE_ENABLED)" NCURSES_ENABLED="$(NCURSES_ENABLED)" SLOPPY_DEPENDENCIES="$(SLOPPY_DEPENDENCIES)" NATIVE_ENABLED="$(NATIVE_ENABLED)" BYTE_ENABLED="$(BYTE_ENABLED)" NATIVE_PROFILING_ENABLED="$(NATIVE_PROFILING_ENABLED)" mk/make_config.sh
